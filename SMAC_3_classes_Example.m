...
Y = [ones(30,1);ones(30,1)*2;ones(30,1)*3]; % Class labels must be from 1 to K.

%% Assign the training, tuning and testing data set.
data.train_x = SIM2.train; % train images, n by p
data.train_y = Y; % train labels, n by 1
data.tune_x = SIM2.tune; % tune images, n by p
data.tune_y = Y; % tune labels, n by 1
data.test_x = SIM2.test; % test images, n by p
data.test_y =  [ones(300,1);ones(300,1)*2;ones(300,1)*3]; % test labels, n by 1
...
data.index = ones(4096,1); 

%% Assign the image size.
imagesize = SIM2.imgsize;  
...
[out1.auc1] = ROC_analysis(data.test_y,out1.prob(:,1),1,1,'SMAC-I');
[out1.auc2] = ROC_analysis(data.test_y,out1.prob(:,2),2,1,'SMAC-I');
[out1.auc3] = ROC_analysis(data.test_y,out1.prob(:,3),3,1,'SMAC-I');
[out2.auc1] = ROC_analysis(data.test_y,out2.prob(:,1),1,1,'SMAC-II');
[out2.auc2] = ROC_analysis(data.test_y,out2.prob(:,2),2,1,'SMAC-II');
[out2.auc3] = ROC_analysis(data.test_y,out2.prob(:,3),3,1,'SMAC-II');
