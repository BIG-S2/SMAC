function [out] = SMAC_II (data,imagesize,options)

% ======================================================================
% ======================================================================
% This algorithm is to solve the Generalized Fused Lasso Penalized
% Multi-Category Angle-Based Classification with Large Margin Unified
% Loss Function.
%
% ======================================================================
% Input:
...
if length(imagesize) > 3
    error('Error: Image size not supported, imagesize must be a vector of length 2 or three')
end
...
GM = GM_gen(d1,d2,d3,k,flag,index);
...
for tune_i = 1:n_lambda1
    for tune_j = 1:n_lambda2
        ...
        if d3>1
            v3 = zeros((10*p_org+1)*(k-1), 1);
        elseif d2 > 1
            v3 = zeros((5*p_org+1)*(k-1), 1);
        else
        ...
        [GB,bccb] = GB_gen(lambda1,lambda2,d1,d2,d3,k);
        ...
        t0 = cputime;
        if options.progress
            fprintf('=================SMAC_II algorithm started===================\n')
        ...
        for t = 1:itmax
            ...
            C_v1=A*b+u1;
            v1=(C_v1<-1).*(1+C_v1)+(ge(C_v1,-1)&le(C_v1,12)).*...
                v1_update_solver(min(floor((abs(C_v1+1))*10^5)+1,1300001))+(C_v1>12).*C_v1;
            ...
            if rel_change  < tol
                fprintf('================SMAC_II algorithm completed==================\n')
                break
            end
